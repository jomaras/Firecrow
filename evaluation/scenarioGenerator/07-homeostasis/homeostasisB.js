var homeostasis = function(id) {
    ...
    var descriptions = {
        ...
        about: 'Drag the model around with the mouse and zoom in and out with the scroll wheel,\n'
            + 'or the \'i\' and \'o\' keys (for \'in\' and \'out\').\n'
        ...
        membrane: 'The membrane is the enclosing surface that separates\n'
            + 'the inside of the cell from the outside.',
        ...
        column: 'The column spans the membrane, allowing for communication\n'
            + 'across the otherwise impenetrable barrier.',
        ...
        cheW: 'cheW is activated by repellents binding to the outer portion\n'
            ...
            + 'methyl groups attached to the inner portion.',
        ...
        cheB: 'cheB is phosphorylated in the presence of active cheW, just like cheY.  cheB\n'
            ...
            + 'the subsequent phosphorylation of cheB (and cheY).',
    ...
    var molecule = function(spec) {
        ...
        that.mouseIn = spec.mouseIn || function(mouse) {
            ...
            if (that.type) {
                moleculeKey.itemhash[that.type] && moleculeKey.itemhash[that.type].activate();
            }
        };
        ...
        that.mouseOut = spec.mouseOut || function(mouse) {
            ...
            if (that.type) {
                moleculeKey.itemhash[that.type] && moleculeKey.itemhash[that.type].deactivate();
            }
        };
...
//                var output = that.type + ' -- ' + that.state + '\n' + that.shape.ops.map(function(s) {return s.to.elements.map(function(x){return Math.round(x);});}) + '\n\n';
                ...
                var keyItem = that.keyItem();
                keyItem && keyItem.showDescription(output);
            }
        };

        var hideDescription = function(mouse) {
            var keyItem = that.keyItem();
            keyItem && keyItem.hideDescription(true);
        };
...
//        spec.orientation = Math.random() * Math.PI;
        ...
        that.columns = $R(0, 2).map(function(index) {
...
//         that.attach(that.flagella);
        ...
        that.phosphates = $R(0, 2).map(function(index) {
            return randomMolecule(phosphate, inside);
        });

        that.methyls = $R(0, 2).map(function(index) {
            return randomMolecule(methyl, inside);
        });

        that.cheYs = $R(0, 2).map(function(index) {
            return randomMolecule(cheY, inside);
        });

        that.cheBs = $R(0, 2).map(function(index) {
            return randomMolecule(cheB, inside);
        });

        that.cheZs = $R(0, 2).map(function(index) {
            return randomMolecule(cheZ, inside);
        });

        that.cheRs = $R(0, 2).map(function(index) {
