  ...
  function FadeTransition(element, opts) {
    ...
    function setup() {
      ...
      if (options.pauseOnMouseOver) {
        ...
        //$('a', options.navButtonContainer|| el || '.classContentBanner').mouseover(pause).mouseout(unpause);
        clearTimeouts();
      }
    ...
    function highlightNav1(nextIdx) {
      if (options.createNavButtons) {
        $('.fadenav a', options.navButtonContainer || el).removeClass('current');
        $('.fadenav a:nth-child(' + (1+nextIdx) + ')', options.navButtonContainer || el).addClass('current');
      }
    }
