...
function draw()
{
    ...
    if (canvas.getContext)
    {
        ...
        for (var i=0; i<contatore; i++)
        {
            ...
            var notSureVar = projected_points[indice_solido][indici_vertici[0]];

            if(notSureVar == null) { continue; }

            ctx.moveTo(notSureVar[0], notSureVar[1]);
            for (var z = 1; z < visible_polygons[i].vertex.length; z++)
            {
                var unknownPoint = projected_points[indice_solido][indici_vertici[z]];

                if(unknownPoint != null)
                {
                    ctx.lineTo(unknownPoint[0], unknownPoint[1]);
                }
            }
