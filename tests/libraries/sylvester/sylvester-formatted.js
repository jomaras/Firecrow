...
Sylvester.Line.Segment.prototype = {
    ...
    translate: function (vector)
    {
        ...
        return Sylvester.Line.Segment.create([S[0] + V[0], S[1] + V[1], S[2] + (V[2] || 0)], [E[0] + V[0], E[1] + V[1], E[2] + (V[2] || 0)]);
    },
    isParallelTo: function (obj)
    {
...
Sylvester.Plane.prototype = {
    ...
    {
        var V = vector.elements || vector;
        return Sylvester.Plane.create([this.anchor.elements[0] + V[0], this.anchor.elements[1] + V[1], this.anchor.elements[2] + (V[2] || 0)], this.normal);
    },
    isParallelTo: function (obj)
    {
        var theta;
    ...
    rotate: function (t, line)
    {
        ...
        var C1 = C[0], C2 = C[1], C3 = C[2], A1 = A[0], A2 = A[1], A3 = A[2];
        var x = A1 - C1, y = A2 - C2, z = A3 - C3;
        return Sylvester.Plane.create([((C1 + (R[0][0] * x)) + (R[0][1] * y)) + (R[0][2] * z), ((C2 + (R[1][0] * x)) + (R[1][1] * y)) + (R[1][2] * z), ((C3 + (R[2][0] * x)) + (R[2][1] * y)) + (R[2][2] * z)], [((R[0][0] * N[0]) + (R[0][1] * N[1])) + (R[0][2] * N[2]), ((R[1][0] * N[0]) + (R[1][1] * N[1])) + (R[1][2] * N[2]), ((R[2][0] * N[0]) + (R[2][1] * N[1])) + (R[2][2] * N[2])]);
    },
    reflectionIn: function (obj)
    {
        if(obj.normal)
    ...
    setVectors: function (anchor, v1, v2)
    {
        ...
        if(v2 !== null)
        {
            ...
            normal = Sylvester.Vector.create([((v12 - A2) * (v23 - A3)) - ((v13 - A3) * (v22 - A2)), ((v13 - A3) * (v21 - A1)) - ((v11 - A1) * (v23 - A3)), ((v11 - A1) * (v22 - A2)) - ((v12 - A2) * (v21 - A1))]);
            mod = normal.modulus();
            if(mod === 0)
            {
                return null;
...
Sylvester.Plane.fromPoints = function (points)
{
    ...
    for(i = 0;i < np;i++)
    {
        ...
        if(n > 2)
        {
            ...
            B = list[n - 2].elements;
            C = list[n - 3].elements;
            N = Sylvester.Vector.create([((A[1] - B[1]) * (C[2] - B[2])) - ((A[2] - B[2]) * (C[1] - B[1])), ((A[2] - B[2]) * (C[0] - B[0])) - ((A[0] - B[0]) * (C[2] - B[2])), ((A[0] - B[0]) * (C[1] - B[1])) - ((A[1] - B[1]) * (C[0] - B[0]))]).toUnitVector();
            if(n > 3)
            {
    ...
    C = list[n - 1].elements;
    D = list[n - 2].elements;
    totalN = totalN.add(Sylvester.Vector.create([((A[1] - B[1]) * (C[2] - B[2])) - ((A[2] - B[2]) * (C[1] - B[1])), ((A[2] - B[2]) * (C[0] - B[0])) - ((A[0] - B[0]) * (C[2] - B[2])), ((A[0] - B[0]) * (C[1] - B[1])) - ((A[1] - B[1]) * (C[0] - B[0]))]).toUnitVector()).add(Sylvester.Vector.create([((B[1] - C[1]) * (D[2] - C[2])) - ((B[2] - C[2]) * (D[1] - C[1])), ((B[2] - C[2]) * (D[0] - C[0])) - ((B[0] - C[0]) * (D[2] - C[2])), ((B[0] - C[0]) * (D[1] - C[1])) - ((B[1] - C[1]) * (D[0] - C[0]))]).toUnitVector());
...
Sylvester.Polygon.prototype = {
    ...
    area: function ()
    {
        if(this.isTriangle())
        {
            ...
            B = B.data.elements;
            C = C.data.elements;
            return 0.5 * Sylvester.Vector.create([((A[1] - B[1]) * (C[2] - B[2])) - ((A[2] - B[2]) * (C[1] - B[1])), ((A[2] - B[2]) * (C[0] - B[0])) - ((A[0] - B[0]) * (C[2] - B[2])), ((A[0] - B[0]) * (C[1] - B[1])) - ((A[1] - B[1]) * (C[0] - B[0]))]).modulus();
        }
        else
        {
    ...
    clearCache: function ()
    {
        this.cached = {triangles: null, surfaceIntegralElements: null};
    },
