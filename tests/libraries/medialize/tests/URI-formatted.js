(function (undefined)
{
    var _use_module = typeof module !== 'undefined' && module.exports, _load_module = function (module)
    {
        return _use_module ? require('./' + module) : window[module];
    }, punycode = _load_module('punycode'), IPv6 = _load_module('IPv6'), SLD = _load_module('SecondLevelDomains'), URI = function (url, base)
    {
    ...
    var _parts = {'encode': 'encode', 'decode': 'decode'}, _part, generateAccessor = function (_part)
    {
    ...
    {
        var pos = string.indexOf('@'), firstSlash = string.indexOf('/'), t;
    ...
    URI.parseQuery = function (string)
    {
        ...
        {
            var v = splits[i].split('='), name = URI.decodeQuery(v.shift()), value = v.length ? URI.decodeQuery(v.join('=')) : null;
            if(items[name])
            {
    ...
    {
        var length = Math.min(one.length, two.length), pos;
    ...
    p.href = function (href, build)
    {
        ...
        else
        {
            ...
            var _URI = href instanceof URI, _object = typeof href === 'object' && (href.hostname || href.path), key;
    ...
    p.is = function (what)
    {
        var ip = false, ip4 = false, ip6 = false, name = false, sld = false, idn = false, punycode = false, relative = !(this._parts.urn);
    ...
    p.suffix = function (v, build)
    {
        ...
        if(v === undefined || v === true)
        {
            ...
            var filename = this.filename(), pos = filename.lastIndexOf('.'), s, res;
            if(pos === -1)
            {
                return '';
            }
        ...
        else
        {
            ...
            var suffix = this.suffix(), replace;
            if(!suffix)
            {
                if(!v)
                {
    ...
    p.normalizePath = function (build)
    {
        ...
        var _was_relative, _was_relative_prefix, _path = this._parts.path, _parent, _pos;
        if(_path[0] !== '/')
        {
            if(_path[0] === '.')
            {
    ...
    p.absoluteTo = function (base)
    {
        ...
        var resolved = new URI(this), properties = ['protocol', 'username', 'password', 'hostname', 'port'], basedir;
    ...
    p.relativeTo = function (base)
    {
        ...
        var relative = new URI(this), properties = ['protocol', 'username', 'password', 'hostname', 'port'], common = URI.commonPath(relative.path(), base.path()), _base = base.directory();
        for(var i = 0, p;p = properties[i];i++)
        {
        ...
        {
            var parents = '../', _common = new RegExp('^' + escapeRegEx(common)), _parents = _base.replace(_common, '/').match(/\//g).length - 1;
            while(_parents--)
            {
                parents += '../';
            }
    ...
    {
        var one = new URI(this), two = new URI(uri), one_map = {}, two_map = {}, checked = {}, one_query, two_query, key;
        one.normalize();
        two.normalize();
    ...
    typeof module !== 'undefined' && module.exports ? module.exports = URI : window.URI = URI;
})();