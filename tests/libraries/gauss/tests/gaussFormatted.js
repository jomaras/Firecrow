...
Vector.prototype = {
    ...
    hmean: function (callback)
    {
        ...
        {
            for(var i = 0, sum = 0;i < set.length;)
            {
    ...
    median: function (callback)
    {
        ...
        var median = (this.vector.length % 2) === 0 ? (buffer.vector[(this.vector.length / 2) - 1] + buffer.vector[this.vector.length / 2]) / 2 : buffer.vector[parseInt(this.vector.length / 2)];
        if(callback)
    ...
    distribution: function (format, callback)
    {
        ...
        var map = function (array, index, format, distribution)
        {
            ...
            {
                distribution[array.vector[index]] = format === 'relative' ? array.frequency(array.vector[index]) / array.vector.length : array.frequency(array.vector[index]);
    ...
    ema: function (options, callback)
    {
        ...
        var sum = (new Vector(this.vector.slice(0, options.period))).sum(), ema = new Vector([sum / options.period]), ratio = options.ratio(options.period);
        for(var i = 1;i < ((this.vector.length - options.period) + 1);i++)
        {
            ema.push((ratio * (this.vector[(i + options.period) - 1] - ema.vector[i - 1])) + ema.vector[i - 1]);
        }
    ...
    push: function ()
    {
        var mean = this.vector.length === 0 ? 0 : this.sum() / this.vector.length;
        var variance = this.vector.length === 0 ? 0 : this.variance() * this.vector.length;
        var args = arguments;
