$(document).ready(function() {
    ...
    if (!$(this).hasClass('active')) {
      ...
      $.get($(this).attr('href'), function(data) {
        $('#news-article').html(data).animate({
          ...
          top: 0,
          marginLeft: 0
        }, {duration: 350, easing: 'easeInOutQuad', complete: function() {
          $(this).find('.entry').height($(this).find('.post').height() - $(this).find('.entry').position().top)
          $(this).find('.post').animate({opacity:1}, 200);
        }});
      })
    }
  });
  ...
  var newsPosition = 0;
  $('#news a.more').animate({bottom:10}, 400).click(function(e) {
    e.preventDefault();
    newsPosition -= 40;
    if (newsPosition < -($('#news ul').height() - 180)) {
      $(this).addClass('back');
      $('#news .thatsall').animate({opacity: 1});
    }
    if (newsPosition < -($('#news ul').height() - 140)) {
      $(this).removeClass('back');
      newsPosition = 0;
    }
    $('#news').find('ul').animate({top: newsPosition}, 300);
  });
  ...
  function gotoPortfolioPage(i) {
    ...
    $('#portfolio-mask .projects').animate({left: -((i - 1) * 360)}, 250);
